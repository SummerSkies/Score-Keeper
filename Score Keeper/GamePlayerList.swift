...
class GamePlayerList: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            tableView.performBatchUpdates({
                GamePlayerList.team.remove(at: indexPath.row)
    ...
    @IBAction func unwindFromDetailView(segue: UIStoryboardSegue) {
        ...
        if let player = sourceViewController.player {
            if let indexOfExistingPlayer = GamePlayerList.team.firstIndex(of: player) {
                //If the selected player already exists in the team array
                GamePlayerList.team[indexOfExistingPlayer] = player
                ...
                let newIndex = IndexPath(row: GamePlayerList.team.count, section: 0)
                GamePlayerList.team.append(player)
    ...
    @IBSegueAction func editPlayer(_ coder: NSCoder, sender: Any?) -> PlayerDetails? {
        ...
        detailView?.player = GamePlayerList.team[indexPath.row]
